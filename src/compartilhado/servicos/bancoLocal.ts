...
export const bancoLocal = {
    ...
    removerPendencia: async (id: string) => {
        const banco = await iniciarBanco();
        await banco.delete('fila_pendencias', id);
    },

    // --- Alunos ---
    salvarAlunos: async (alunos: AlunoLocal[], sincronizado = 1) => {
        const banco = await iniciarBanco();
    ...
    deletarAluno: async (matricula: string) => {
        const banco = await iniciarBanco();
        await banco.delete('alunos', matricula);
    },

    // --- Turmas ---
    salvarTurmas: async (turmas: TurmaLocal[], sincronizado = 1) => {
        const banco = await iniciarBanco();
    ...
    importarAlunos: async (novosAlunos: AlunoLocal[]) => {
        const banco = await iniciarBanco();
        const transacao = banco.transaction('alunos', 'readwrite');
        await Promise.all([
    ...
    buscarAluno: async (matricula: string) => {
        const banco = await iniciarBanco();
        return await banco.get('alunos', matricula);
    },

    buscarAlunosPorNome: async (termo: string) => {
        const banco = await iniciarBanco();
    ...
    marcarComoSincronizado: async (ids: string[] | number[]) => {
        const banco = await iniciarBanco();
    ...
    salvarUsuario: async (usuario: UsuarioLocal) => {
        const banco = await iniciarBanco();
    ...
    deletarUsuario: async (email: string) => {
        const banco = await iniciarBanco();
    ...
    marcarLogsComoSincronizados: async (ids: string[] | number[]) => {
        const banco = await iniciarBanco();
    ...
    obterRegistrosPeriodo: async (dataInicio: Date, dataFim: Date) => {
        const banco = await iniciarBanco();
