...
const obterDataCorrigida = () => new Date();
const obterVersaoAtual = () => '1.0';
export const TIPOS_CONSENTIMENTO = { COLETA_DADOS: 'coleta_dados' };
 ...
 */
export interface ParametrosConsentimento {
 ...
 */
export const verificarConsentimentoValido = async (alunoMatricula: string): Promise<boolean> => {
    const banco = await bancoLocal.iniciarBanco();
 ...
 */
export const retirarConsentimento = async (consentimentoId: string, retiradoPor: string | null = null) => {
    const banco = await bancoLocal.iniciarBanco();
 ...
 */
export const obterHistoricoConsentimentos = async (alunoMatricula: string) => {
    const banco = await bancoLocal.iniciarBanco();
    ...
    consentimentos.sort((a, b) =>
        new Date(b.data_consentimento).getTime() - new Date(a.data_consentimento).getTime()
    );
