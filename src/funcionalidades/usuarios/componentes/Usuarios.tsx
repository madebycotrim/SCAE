...
export default function Usuarios() {
    ...
    const carregarUsuarios = async () => {
        try {
            ...
            log.error('Erro ao carregar usuÃ¡rios', e);
    ...
    const salvarUsuario = async () => {
        ...
        try {
            const novoUsuario: UsuarioLocal = {
                ...
                papel: dadosFormulario.role.toUpperCase() as PapelUsuario,
                ativo: true,
            ...
            log.error('Erro ao salvar usuÃ¡rio', erro);
    ...
    const toggleStatus = async (user: UsuarioLocal) => {
        try {
            ...
            if (navigator.onLine) {
                try {
                    // Reenviar usuÃ¡rio com status atualizado (Sanitizado)
                    const payload = {
            ...
            // Atualiza lista localmente para feedback instantÃ¢neo
            ...
            toast.success(`UsuÃ¡rio ${usuarioAtualizado.ativo ? 'ativado' : 'desativado'}!`);
        } catch (erro) {
    ...
    const excluirUsuario = async (user: UsuarioLocal) => {
        if (!window.confirm(`Tem certeza que deseja EXCLUIR DEFINITIVAMENTE o usuÃ¡rio ${user.email}? Essa aÃ§Ã£o nÃ£o pode ser desfeita.`)) {
            return;
        }
        ...
        try {
            ...
            log.error('Erro ao excluir usuÃ¡rio', erro);
            toast.error('Erro ao excluir usuÃ¡rio.');
        }
    };

    const abrirEdicao = (user: UsuarioLocal) => {
        definirUsuarioEmEdicao(user);
        // Mapear 'papel' do banco (ADMIN) para 'role' do form (admin) se necessÃ¡rio, ou usar direto
    ...
    const PapeisDisponiveis = [
        ...
        { id: 'COORDENACAO', nome: 'CoordenaÃ§Ã£o', desc: 'GestÃ£o pedagÃ³gica' },
        { id: 'SECRETARIA', nome: 'Secretaria', desc: 'GestÃ£o de alunos e turmas' },
        { id: 'PORTARIA', nome: 'Portaria', desc: 'Apenas registro de acesso' },
        { id: 'VISUALIZACAO', nome: 'Visitante', desc: 'Apenas visualizaÃ§Ã£o' }
    ];

    return (
        <LayoutAdministrativo
            titulo="Gerenciamento de UsuÃ¡rios"
            subtitulo="Controle de acesso e permissÃµes"
            acoes={
                <div className="flex gap-2">
                    ...
                    >
                        <Plus size={20} /> <span className="hidden sm:inline">Convidar UsuÃ¡rio</span>
                    </button>
                </div>
            }
        >
            ...
            {modalAberto && (
                <ModalUniversal
                    titulo={usuarioEmEdicao ? "Gerenciar PermissÃµes" : "Convidar Novo UsuÃ¡rio"}
                    subtitulo={usuarioEmEdicao ? "Altere o nÃ­vel de acesso ou status deste usuÃ¡rio." : "Adicione um novo membro Ã  equipe administrativa."}
                    fechavel
                    tamanho="lg"
                ...
                >
                    <div className="space-y-6">
                        ...
                        {/* Roles Grid */}
                        <div>
                            <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-3">Defina a FunÃ§Ã£o</label>
                        ...
                        <div className="flex gap-4 pt-6 mt-2 border-t border-slate-100">
                            ...
                            >
                                ...
                                {usuarioEmEdicao ? 'Salvar AlteraÃ§Ãµes' : 'Convidar UsuÃ¡rio'}
                            </button>
                        </div>
                    </div>
                </ModalUniversal>
