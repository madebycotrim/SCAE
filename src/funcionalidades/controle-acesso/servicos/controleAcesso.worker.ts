...
async function drenarFila(authHeader: string, idEscola: string) {
    if (isSyncing) return;
    try {
        isSyncing = true;
        ...
        const idsSincronizados = await processarLote(lote, authHeader, idEscola);
        ...
        if (idsSincronizados.length > 0) {
            ...
            isSyncing = false;
            drenarFila(authHeader, idEscola);
        }
...
self.addEventListener('message', async (e: MessageEvent) => {
    const { tipo, authHeader, idEscola } = e.data;

    // Sinal de Acordar do Hook `usarControleAcessoWorker`
