...
export default function Login() {
    ...
    // Estado removido pois o login admin agora Ã© direto
    ...
    const handleGoogleLogin = async (tipo: 'admin' | 'user') => {
        ...
        try {
            ...
            // Para admin: sugere o email especÃ­fico. Para usuÃ¡rios: restringe ao domÃ­nio institucional.
            ...
            if (tipo === 'admin') {
                if (email !== EMAIL_ADMIN_RAIZ) {
                    await sair();
                    throw new Error('ACESSO NEGADO: Este email nÃ£o tem permissÃ£o de administrador.');
                }
                toast.success('Bem-vindo, Administrador!');
            } else {
                // Permite APENAS @edu.se.df.gov.br (Email Institucional Exclusivo)
                // Opcionalmente ainda validamos aqui caso o 'hd' seja burlado ou o usuÃ¡rio use outra conta
                const dominioValido = email.endsWith(DOMINIO_INSTITUCIONAL);

                if (!dominioValido) {
                    await sair();
                    throw new Error(`ACESSO NEGADO: Apenas emails institucionais (${DOMINIO_INSTITUCIONAL}) sÃ£o permitidos.`);
                }
            ...
            // Tratamento de erros do Firebase ou ValidaÃ§Ã£o
            let mensagem = error.message;
            if (error?.code === 'auth/popup-closed-by-user') mensagem = 'Login cancelado pelo usuÃ¡rio.';
            if (error?.code === 'auth/network-request-failed') mensagem = 'Erro de conexÃ£o. Verifique sua internet.';

            definirErro(mensagem);
            toast.error(mensagem);
            await sair(); // Garante logout em caso de erro pÃ³s-auth
        } finally {
                    ...
                    <div className="md:w-1/2 p-10 md:p-14 bg-[#0f172a] relative flex flex-col justify-between overflow-hidden">
                        ...
                        <div className="relative z-10 space-y-6">
                            ...
                            <p className="text-slate-400 text-lg leading-relaxed max-w-sm">
                                Conectando seguranÃ§a, controle de acesso e frequÃªncia em uma Ãºnica plataforma unificada.
                            </p>
