...
export default function TelaAcesso() {
    ...
    const handleGoogleLogin = async (tipo: 'admin' | 'user') => {
        ...
        try {
            ...
            // Se houver domínio configurado na escola, restringe a ele.
            const params = dominioEmail ? { hd: dominioEmail } : {};

            const resultado = await entrar(params) as { user: { email: string } };
            const usuario = resultado.user;
            const email = usuario.email;

            // 2. Validação de Segurança (O Middleware do Cloudflare fará a checagem real no banco)
            if (dominioEmail && !email.endsWith(dominioEmail)) {
                await sair();
                        ...
                        <div className="max-w-xs mx-auto w-full">
                            ...
                            <p className="mt-6 text-center text-[13px] text-slate-500 font-medium">
                                Utilize seu e-mail institucional {dominioEmail && (
                                    <>da escola: <br /><strong className="text-slate-700 break-all">@{dominioEmail}</strong></>
                                )}
                            </p>
