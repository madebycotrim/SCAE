...
import SelectComBusca from './SelectComBusca';
import {
    Users,
    Search,
    Plus,
...
const log = criarRegistrador('Alunos');
import { Registrador } from '@compartilhado/servicos/auditoria';


export default function Alunos() {
    const { dados, carregando, recarregar: carregarDados } = useConsulta(
        ['alunos-e-turmas'],
        async () => {
            ...
            return { alunos: listaAlunos, turmas: listaTurmas };
        }
    );
    ...
    const importarDados = async (jsonData) => {
        ...
        // Obter matrÃ­culas jÃ¡ existentes
        const matriculasExistentes = new Set(alunos.map(a => a.matricula));

        for (const row of jsonData) {
            // Normalizar chaves para suportar visualizaÃ§Ã£o de array (colar) ou objeto (arquivo)
            ...
            if (Array.isArray(row)) {
                ...
                matricula = row['Matricula'] || row['MatrÃ­cula'] || row['matricula'];
            ...
            if (!aluno.nome_completo || !aluno.matricula) {
                // NÃ£o contar linha vazia como erro se for totalmente vazia
            ...
            if (matriculasExistentes.has(aluno.matricula)) {
                erros++;
                errosDetalhes.push(`MatrÃ­cula duplicada: ${aluno.matricula} (${aluno.nome_completo})`);
                continue;
            }
    ...
    const processarArquivo = async (e) => {
        ...
        try {
            ...
            // Sheet to json padrÃ£o gera objetos
            ...
            log.error('Erro na importaÃ§Ã£o', erro);
    ...
    const processarColagem = async () => {
        ...
        try {
            ...
            // Remover cabeÃ§alho se houver (se a primeira linha parecer cabeÃ§alho)
            // LÃ³gica simples: se primeira coluna for "Nome..." ignorar
            if (data.length > 0) {
            ...
            log.error('Erro na colagem', erro);
    ...
    const salvarAluno = async () => {
        ...
        try {
            ...
            if (!alunoEmEdicao) {
                ...
                if (existente) {
                    toast.error('MatrÃ­cula jÃ¡ cadastrada!');
                    return;
                }
            }
            ...
            if (navigator.onLine) {
                ...
                } catch (e) { log.warn('Sync nuvem falhou', e); }
            }
    ...
    const confirmarExclusao = async () => {
        ...
        try {
            ...
            // toast.success jÃ¡ Ã© chamado dentro de excluirAluno? NÃ£o, ele chama carregarAlunos que nÃ£o tem toast de sucesso de exclusÃ£o.
    ...
    // LÃ³gica de PaginaÃ§Ã£o e Filtro
    ...
    return (
        <LayoutAdministrativo titulo="GestÃ£o de Alunos" subtitulo="Cadastro e controle de matrÃ­culas" acoes={AcoesHeader}>
                ...
                >
                    <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            {/* MatrÃ­cula */}
                            <div>
                                <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">MatrÃ­cula</label>
                                <div className="relative">
                                    <input
                                        type="text"
                        ...
                        <div className="flex gap-4 pt-6 mt-2 border-t border-slate-100">
                            ...
                            >
                                ...
                                {alunoEmEdicao ? 'Salvar AlteraÃ§Ãµes' : 'Cadastrar Aluno'}
                            </button>
                        </div>
                    </div>
                </ModalUniversal>
            ...
            {modalQRCode && (
                ...
                >
                    <div className="flex flex-col items-center justify-center p-6 bg-white rounded-xl border border-slate-100 shadow-inner">
                        ...
                        <p className="text-sm font-bold text-slate-800 mb-1">MatrÃ­cula: {qrcodeAtual}</p>
                        <p className="text-xs text-slate-400">Escaneie para registrar acesso</p>
                    </div>
                </ModalUniversal>
            )}

            {/* Modal ImportaÃ§Ã£o Premium */}
            {modalImportacao && (
                <ModalUniversal
                    titulo="ImportaÃ§Ã£o em Massa"
                    subtitulo="Selecione o arquivo ou cole os dados para cadastrar mÃºltiplos alunos."
                    aoFechar={() => {
                        ...
                        {!resultadoImportacao ? (
                            <div className="space-y-6">
                                            ...
                                            >
                                                ...
                                                <div className={`relative bg-white border-2 ${isDragging ? 'border-indigo-500 bg-indigo-50/30 border-dashed' : 'border-slate-200 border-dashed hover:border-indigo-300 hover:bg-slate-50'} rounded-2xl p-10 flex flex-col items-center justify-center text-center transition-all min-h-[300px]`}>
                                                    ...
                                                    <p className="text-slate-400 mb-6 max-w-xs mx-auto leading-relaxed">
                                                        Suportamos arquivos .xlsx, .xls e .csv. <br /> O processamento Ã© instantÃ¢neo.
                                                    </p>
                                            ...
                                            <div className="flex flex-col h-full min-h-[300px]">
                                                <div className="relative flex-1 bg-white border border-slate-200 rounded-2xl focus-within:ring-2 focus-within:ring-indigo-500/20 focus-within:border-indigo-500 transition-all overflow-hidden group shadow-sm">
                                                    ...
                                                    <textarea
                                                        ...
                                                        placeholder={`Nome Completo\tMatrÃ­cula\tTurma\nJoÃ£o Silva\t2023001\t3A\nMaria Souza\t2023002\t3B`}
                                ...
                                <div className="flex flex-col sm:flex-row items-center justify-between gap-4 px-2 pt-2 border-t border-slate-50">
                                    <div className="flex items-start gap-2 text-xs text-slate-500 max-w-sm">
                                        ...
                                        <span>
                                            Certifique-se que as colunas sÃ£o: <span className="font-mono text-slate-700 bg-slate-100 px-1 rounded">Nome</span>, <span className="font-mono text-slate-700 bg-slate-100 px-1 rounded">MatrÃ­cula</span> e <span className="font-mono text-slate-700 bg-slate-100 px-1 rounded">Turma</span>.
                                        </span>
                                    </div>
                            ...
                            <div className="animate-in fade-in zoom-in duration-300">
                                ...
                                <div className="text-center mb-8">
                                    ...
                                    <h3 className="text-xl font-bold text-slate-800">Processamento ConcluÃ­do</h3>
                                    <p className="text-slate-500 text-sm">Confira o resumo da operaÃ§Ã£o abaixo.</p>
                                </div>
            ...
            {/* Modal de ExclusÃ£o */}
            {
                modalExclusaoAberto && alunoParaExcluir && (
                    <ModalUniversal
                        ...
                        subtitulo="Esta aÃ§Ã£o Ã© irreversÃ­vel e removerÃ¡ todos os dados deste aluno."
                        icone={Trash2}
                        cor="rose"
                        tamanho="sm"
                    >
                        <div className="p-4">
                            <p className="text-slate-600 mb-6 leading-relaxed">
                                VocÃª estÃ¡ prestes a excluir o aluno <strong className="text-slate-900">{alunoParaExcluir.nome_completo}</strong> (MatrÃ­cula: {alunoParaExcluir.matricula}).
                                <br /><br />
