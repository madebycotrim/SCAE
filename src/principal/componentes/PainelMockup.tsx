                                ...
                                <div className="grid grid-cols-3 gap-4">
                                    ...
                                    <div className={`h-28 rounded-2xl p-5 flex flex-col justify-between border relative overflow-hidden group transition-all duration-500 ${temaEscuro ? 'bg-slate-800/80 border-slate-700 hover:border-slate-500/30 hover:shadow-[0_0_20px_rgba(100,116,139,0.1)]' : 'bg-white border-slate-100 shadow-suave hover:border-slate-300 hover:shadow-[0_0_20px_rgba(100,116,139,0.05)]'}`}>
                                        ...
                                        <div className={`text-[11px] font-semibold flex items-center gap-1.5 relative z-10 mt-1 ${temaEscuro ? 'text-slate-400' : 'text-slate-500'}`}>
                                            <div className={`w-4 h-4 rounded-full flex items-center justify-center ${temaEscuro ? 'bg-slate-700' : 'bg-slate-100'}`}>
                                                <div className={`w-1.5 h-1.5 rounded-full ${temaEscuro ? 'bg-slate-400' : 'bg-slate-400'}`}></div>
                                            </div>
                                    ...
                                    <div className={`h-28 rounded-2xl p-5 flex flex-col justify-between border relative overflow-hidden group transition-all duration-500 ${temaEscuro ? 'bg-slate-800/80 border-slate-700 hover:border-[#0d1f3c]/50 hover:shadow-[0_0_20px_rgba(13,31,60,0.3)]' : 'bg-white border-slate-100 shadow-suave hover:border-sky-200 hover:shadow-[0_0_20px_rgba(56,189,248,0.1)]'}`}>
                                        ...
                                        <div className={`text-[10px] font-bold flex flex-col justify-center relative z-10 mt-1 ${temaEscuro ? 'text-sky-300' : 'text-sky-600'}`}>
                                            <div className={`w-full h-1.5 rounded-full overflow-hidden mt-0.5 ${temaEscuro ? 'bg-slate-700/50' : 'bg-slate-100'}`}>
                                                <div className={`h-full w-[98%] relative overflow-hidden ${temaEscuro ? 'bg-sky-400' : 'bg-[#0d1f3c]'}`}>
                                ...
                                <div className={`flex-1 rounded-2xl border p-5 flex flex-col gap-4 relative overflow-hidden group transition-all duration-500 ${temaEscuro ? 'bg-slate-800/50 border-slate-700 hover:border-slate-600 hover:shadow-[0_0_30px_rgba(0,0,0,0.3)]' : 'bg-white border-slate-100 shadow-suave hover:border-sky-100 hover:shadow-[0_0_30px_rgba(99,102,241,0.05)]'}`}>
                                    ...
                                    <div className="flex items-center justify-between relative z-10">
                                        ...
                                        <div className="flex gap-1.5">
                                            <div className={`w-1.5 h-1.5 rounded-full ${temaEscuro ? 'bg-sky-500' : 'bg-sky-500'}`}></div>
                                    ...
                                    <div className="flex-1 flex items-end justify-between gap-1.5 sm:gap-3 pt-2 relative z-10 pb-1">
                                        {[40, 70, 45, 95, 65, 30, 80].map((h, i) => (
                                            <div key={i} className="flex-1 flex flex-col items-center gap-2.5 group/bar cursor-default h-full justify-end">
                                                <div
                                                    className={`w-full max-w-[40px] rounded-t-lg transition-all duration-700 ease-out relative overflow-hidden ${temaEscuro ? 'bg-sky-500/40 group-hover/bar:bg-sky-400 group-hover/bar:shadow-[0_0_15px_rgba(99,102,241,0.5)]' : 'bg-sky-200 group-hover/bar:bg-sky-500 group-hover/bar:shadow-[0_0_15px_rgba(99,102,241,0.4)]'}`}
                                                    style={{ height: `${h}%` }}
                                                >
            ...
            >
                <div className={`w-full rounded-[2.5rem] border-[8px] p-3 shadow-2xl relative overflow-hidden transition-all duration-500 ${temaEscuro ? 'bg-[#0B0F19] border-slate-800 shadow-[0_20px_50px_rgb(0,0,0,0.5)] shadow-sky-500/10' : 'bg-slate-50 border-slate-800 shadow-[0_20px_50px_rgb(0,0,0,0.2)]'}`}>
                    ...
                    <div className="pt-6 pb-4 flex justify-between items-center px-1">
                        ...
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center ${temaEscuro ? 'bg-slate-800 text-sky-400' : 'bg-white shadow-suave text-sky-600'}`}>
                    ...
                    <div className="flex flex-col gap-3">
                        {/* Entradas Card */}
                        <div className={`rounded-2xl p-4 flex flex-col justify-between border relative overflow-hidden ${temaEscuro ? 'bg-slate-800/80 border-slate-700' : 'bg-white border-slate-100 shadow-suave'}`}>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            ...
                            <div className={`rounded-xl p-3 flex flex-col justify-between border relative overflow-hidden ${temaEscuro ? 'bg-slate-800/80 border-slate-700' : 'bg-white border-slate-100 shadow-suave'}`}>
                                ...
                                <div className={`w-full h-1 rounded-full overflow-hidden mt-1.5 ${temaEscuro ? 'bg-slate-700/50' : 'bg-slate-100'}`}>
                                    <div className={`h-full w-[98%] ${temaEscuro ? 'bg-sky-400' : 'bg-[#0d1f3c]'}`}></div>
                                </div>
                            </div>
                        </div>

                        {/* Tiny Chart */}
                        <div className={`rounded-2xl border p-4 flex flex-col gap-3 relative overflow-hidden h-32 ${temaEscuro ? 'bg-slate-800/50 border-slate-700' : 'bg-white border-slate-100 shadow-suave'}`}>
                            ...
                            <div className="flex-1 flex items-end justify-between gap-1 pt-1">
                                {[40, 70, 45, 95, 65, 30, 80].map((h, i) => (
                                    <div key={i} className="flex-1 flex flex-col items-center h-full justify-end">
                                        <div
                                            className={`w-full rounded-t-sm transition-all duration-700 relative overflow-hidden ${temaEscuro ? 'bg-sky-500/40' : 'bg-sky-200'}`}
                                            style={{ height: `${h}%` }}
                                        ></div>
                                    </div>
                                ))}
                    ...
                    <div className={`mt-4 pt-2 pb-1 border-t flex justify-around ${temaEscuro ? 'border-slate-800 text-slate-500' : 'border-slate-200 text-slate-400'}`}>
                        <div className={`w-5 h-5 rounded ${temaEscuro ? 'bg-sky-500/20 text-sky-400' : 'bg-sky-100 text-sky-600'}`}></div>
