...
export async function onRequestGet(contexto: ContextoCatraki): Promise<Response> {
    try {
        ...
        const [alunosRes, turmasRes, alertasRes, registrosHjRes, historicoRes, alunosFeedRes] = await contexto.env.DB_SCAE.batch([
            ...
            contexto.env.DB_SCAE.prepare("SELECT aluno_matricula, tipo_movimentacao, timestamp_acesso as timestamp FROM registros_acesso WHERE escola_id = ? AND (date(timestamp_acesso, '-3 hours') = ? OR timestamp_acesso >= datetime('now', '-6 hours'))").bind(idEscola, hojeStr),
